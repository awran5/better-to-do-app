...
const Task = ({ task }) => {
  ...
  return (
    <li className="list-item">
      ...
      <div>
        <button type="button" title="Edit" className="btn-edit task-btn" onClick={() => findItem(task.id)}>
        ...
        <button type="button" title="Delete" className="btn-delete task-btn" onClick={() => removeTask(task.id)}>
          <svg fill="#eee" className="icon" viewBox="0 0 24 24">
        ...
        >
          <svg fill="#eee" className={`icon ${task.complete ? 'complete' : ''}`} viewBox="0 0 24 24">
